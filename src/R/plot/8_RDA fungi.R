...
fungi <- read.csv("./data/result/micro/mean/fungi genus top20.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
fungi <- fungi[, -ncol(fungi)]

uu <- rda(physicochemical, fungi, scale = T) #RDA分析
...
ggsave(filename = "./pdf/RDA/fungi.pdf", g, width = 18, height = 15, dpi = 600, units = "cm", device = 'pdf')
