...
bacteria <- read.csv("./data/temp/microbe/chamber/merge/top/traditional.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
physicochemical <- read.csv("./data/temp/phy/chamber/mean/traditional.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
mapping <- read.csv("./data/mapping/chamber/traditional.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
...
g <- ggplot() +
  ...
  geom_segment(data = df_rda_env, aes(x = 0, y = 0, xend = df_rda_env[, 1], yend = df_rda_env[, 2]),
               color = "#A6761D", linetype = 1, linewidth = .8, arrow = arrow(angle = 35, length = unit(0.3, "cm"))) +
  geom_text_repel(data = df_rda_env,
                  aes(RDA1, RDA2, label = name), size = 2,
  ...
  geom_point(data = df_rda_species, aes(RDA1, RDA2), color = "#605399", size = 1) +
  geom_text_repel(data = df_rda_species,
                  aes(RDA1, RDA2, label = rownames(df_rda_species)), size = 1.5,
                  box.padding = 0.2, #字到点的距离
                  point.padding = 0.2, #字到点的距离，点周围的空白宽度
                  min.segment.length = 0.5, #短线段可以省略
                  segment.color = "#545454",
...
ggsave(filename = "./pdf/rda/tradiction.pdf", g, width = 9, height = 8, dpi = 600, units = "cm", device = 'pdf')
