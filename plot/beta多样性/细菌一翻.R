...
data <- read.csv("./data/temp/beta/bacteria/YF.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
mapping <- read.csv("./data/mapping/chamber/complete.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
mapping <- mapping[grep("^B", row.names(mapping)),]
...
ggsave(filename = "./pdf/beta/bacteria/YF.pdf", g, width = 9, height = 8, dpi = 600, units = "cm", device = 'pdf')
