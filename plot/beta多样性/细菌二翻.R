...
data <- read.csv("./data/temp/beta/bacteria/EF.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
mapping <- read.csv("./data/mapping/chamber/complete.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
mapping <- mapping[grep("^C", row.names(mapping)),]
...
ggsave(filename = "./pdf/beta/bacteria/EF.pdf", g, width = 9, height = 8, dpi = 600, units = "cm", device = 'pdf')
