...
library(forcats)
source("./plot/font.R")
data <- read.csv("./data/temp/abundance/fungi/mechanical.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
...
g <- ggplot(data, aes(x = Samples, y = Values, fill = Strains)) +
  ...
  guides(fill = 'none') +
  labs(x = "", y = "相对丰度 (%)")
ggsave(filename = "./pdf/abundance/fungi/mechanical/YF.pdf", g, width = 14, height = 8, dpi = 600, units = "cm", device = 'pdf')
