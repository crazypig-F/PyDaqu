...
library(forcats)
source("./plot/font.R")
amino <- read.csv("./data/temp/amino/stage/mean/mechanical.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
mapping <- read.csv("./data/mapping/stage/mechanical.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
...
ggsave(filename = "./pdf/amino/mechanical.pdf", g, width = 12, height = 8, dpi = 600, units = "cm", device = 'pdf')
