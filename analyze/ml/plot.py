...
def plot_feature(name, model, feat_labels):
    ...
    pd.DataFrame({"Value": values, "Name": index}, index=index).to_csv(f"./data/temp/ml/feature_{name}.csv")
    ...
    # plt.show()
    plt.savefig(f'./pdf/ml/feature/phase_{name}.pdf', dpi=600)
    plt.close()
